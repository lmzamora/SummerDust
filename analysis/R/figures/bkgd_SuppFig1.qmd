...
foofo<-foo
names(foofo)<-c("lonbin","latbin","zbin","n_flex","meandust_flex","mean_overlap_flex","fmid_flex","area")
```
...
foo[,area:=area.helper(lonbin,latbin,size.lon,size.lat)*1e-6]
foomo<-foo
names(foomo)<-c("lonbin","latbin","zbin","n_mer","meandust_mer","mean_overlap_mer","fmid_mer","area")
```

Join the two data sets together:

```{r}
sfig1<-full_join(foomo, foofo, by=c("lonbin", "latbin", "zbin","area"))
fwrite(sfig1,file="../../data/derived_data/SFig1.csv",na=NA,quote=F)
```
