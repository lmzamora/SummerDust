...
Add some new variables to the dataset that listed at every observation, but that show the sum, sample number, or mean of the variable within that observation's meteorological bin as defined by the 3 variables. For every observation, add relevant information about the characteristics of the meteorological bin it belonged to:
...
Add some new variables to the dataset that listed at every observation, but that show the sum, sample number, or mean of the variable within that observation's meteorological bin as defined by the 3 variables and within the 400 km2 grid cell the observation falls in. This includes

```{r}
	la2[,alln:=sum(all,na.rm=T),by=.(var1bin,var2bin,var3bin,gridcell)] #number of aerosol observations in the ij bin
	la2[,cleann:=sum(clean,na.rm=T),by=.(var1bin,var2bin,var3bin,gridcell)] #number of times aerosol conditions are clean in the ij bin
	...
	for(j in 1:length(groups)) {
		eval(parse(t=paste0("la2[,d",groups[j],":=mean(",groups[j],"[test.this==1], na.rm=T)-mean(",groups[j],"[cleancloud==1], na.rm=T), by=.(var1bin,var2bin,var3bin,gridcell)]")))
	} 
```
