...
For **Figure 1a** first load the following libraries:
...
```{r}
#load("points_all_orig_aerosols_v7.RData")
#phase<-merged2[,c(1:8, 35, 125, 175:182)]
#setDT(phase)
#phase<-phase[ocean==1&seaice>0.9]
#fwrite(phase,file="../../data/derived_data/phase.csv", na=NA)
#phase<-read.csv("/Users/lzamora/Library/CloudStorage/OneDrive-NASA/NASA_Science/8000-5_R+D_project_files/ACMAP3/initial_data/all/phase.csv")
#setDT(phase)
```

Get percent of clouds in different phases at different altitudes:

```{r}

for(i in 1:8) { #For each altitude level of interest
   eval(parse(t=paste0("fCice_lev",i," <- length(na.exclude(phase$CP",i,"[phase$CP",i,"==1]))/length(na.exclude(phase$CP",i,"))*100")))
   eval(parse(t=paste0("fCmpc_lev",i," <- length(na.exclude(phase$CP",i,"[phase$CP",i,"==2]))/length(na.exclude(phase$CP",i,"))*100")))
   eval(parse(t=paste0("fCliq_lev",i," <- length(na.exclude(phase$CP",i,"[phase$CP",i,"==3]))/length(na.exclude(phase$CP",i,"))*100")))
}
fCice<-c(fCice_lev1, fCice_lev2, fCice_lev3, fCice_lev4, fCice_lev5, fCice_lev6, fCice_lev7, fCice_lev8)
fCmpc<-c(fCmpc_lev1, fCmpc_lev2, fCmpc_lev3, fCmpc_lev4, fCmpc_lev5, fCmpc_lev6, fCmpc_lev7, fCmpc_lev8)
fCliq<-c(fCliq_lev1, fCliq_lev2, fCliq_lev3, fCliq_lev4, fCliq_lev5, fCliq_lev6, fCliq_lev7, fCliq_lev8)

altitude<-c(mean(c(0.6,1.5)), 2:8)
fC<-cbind(altitude,fCice,fCmpc,fCliq)

```

Save the files needed to plot **Figure 1a**.

```{r}
fwrite(fC,file="../../data/derived_data/fC.csv")
...
del1<-la3[unic>=min4&unia>=min4&cleanncloud>=min & notcleanncloud>=min , 
		.(
		dglaciation=mean(fi[test.this==1],na.rm=T)-mean(fi[cleancloud==1], na.rm=T),
		dcloud_thickness=mean(th[test.this==1], na.rm=T)-mean(th[cleancloud==1], na.rm=T),
...
del2<-expand.grid(var1bin=min(del1$var1bin,na.rm=T):max(del1$var1bin,na.rm=T),var3bin=min(del1$var3bin,na.rm=T):max(del1$var3bin,na.rm=T),dglaciation=NA,dcloud_thickness=NA,alln=NA,var2bin=NA) 
...
del1p<-la3[unic>=min4&unia>=min4&cleanncloud>=min & notcleanncloud>=min , 
		.(
		dglaciationp=as.numeric(try(wilcox.test(fi[test.this==1],fi[cleancloud==1])$p.value)),
		dcloud_thicknessp=as.numeric(try(wilcox.test(th[test.this==1], th[cleancloud==1])$p.value))
				) ,
		by=.(var1bin,var3bin)] 
# Add in the missing bins for plotting purposess
del2p<-expand.grid(var1bin=min(del1p$var1bin,na.rm=T):max(del1p$var1bin,na.rm=T),var3bin=min(del1p$var3bin,na.rm=T):max(del1p$var3bin,na.rm=T),dglaciationp=NA,dcloud_thicknessp=NA)
...
del2p[,merT:=var1range[var1bin]]
del2p[,merQV:=var3range[var3bin]]
...
```{r}
#test<-del2[, weighted.mean(dglaciation,alln,na.rm=T), by=.(var1bin,var3bin)][, dcast(.SD, var1bin~var3bin)]    
...
```{r}
colnames(del2)<-c('merTbin', 'tempbin', 'merQVbin', 'dglaciation', 'dcloudthickness', 'alln', 'merT', 'temp', 'merQV')
colnames(del2p)<-c('merTbin', 'merQVbin', 'dglaciationp', 'dcloudthicknessp', 'merT', 'merQV')
fwrite(del2, file="../../data/derived_data/del2.csv",na=NA,quote=F)
fwrite(del2p, file="../../data/derived_data/del2p.csv",na=NA,quote=F)
