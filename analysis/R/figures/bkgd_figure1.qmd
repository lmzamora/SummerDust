...
#  eval(parse(t=paste0("load('/Users/lzamora/Library/CloudStorage/OneDrive-NASA/NASA_Science/8000-5_R+D_project_files/ACMAP3/initial_data/all/old/lev",i,"_v3.RData')")))
#        la2<-eval(parse(t=paste0("lev",i)))
#        eval(parse(t=paste0("rm(lev",i,")")))
#        setDT(la2)
#        la2<-la2[ocean==1&seaice>=0.9,]
#        fCice=length(na.exclude(la2$CP[la2$CP==1])) / length(na.exclude(la2$CP))
#        fCmpc=length(na.exclude(la2$CP[la2$CP==2])) / length(na.exclude(la2$CP))
#       fCliq=length(na.exclude(la2$CP[la2$CP==3])) / length(na.exclude(la2$CP))
#        eval(parse(t=paste0("fCice_lev",i,"<- fCice")))
#        eval(parse(t=paste0("fCmpc_lev",i,"<- fCmpc")))
#        eval(parse(t=paste0("fCliq_lev",i,"<- fCliq")))
#        rm(la2)
#}
load("/Users/lzamora/Library/CloudStorage/OneDrive-NASA/NASA_Science/8000-5_R+D_project_files/ACMAP3/initial_data/all/points_all_orig_aerosols_v7.RData")
